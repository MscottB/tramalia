...
[![Tests](https://img.shields.io/badge/tests-34%20passing-brightgreen.svg)](#status--roadmap)
[![Status](https://img.shields.io/badge/status-v0.6%20preview-orange.svg)](#status--roadmap)
...
- 🏛️ **Governed close (`close`)** — runs the gates, writes their output into the **evidence pack** and generates the **handoff** in one step; **blocks the close if a gate fails** (unless a documented exception).
...
External tools are **optional interoperability**, not requirements: `mise` (runs the gates), Repomix/Serena/codebase-memory-mcp (context), rulesync (fan-out), **Engram** (N2 memory), **Headroom** (compression). If they're missing, Tramalia still governs and records it as a documented exception. So you can work **with Tramalia alone** or **combine it** with the whole ecosystem.
...
- 🌐 **Site (ES/EN):** https://MscottB.github.io/tramalia/
...
Together: **Gentle-AI** enables *which* agents to work with, **Engram** helps *remember*, **Headroom** makes context *cheaper*, **Serena/Repomix/codebase-memory-mcp** provide *code intelligence*, and **Tramalia** keeps the repo **controlled, traceable and consistent**. All are optional interop; none touch Tramalia's core (`close`, `log`, evidence pack, handoff). In particular, **Headroom never replaces evidence**: the raw output is always kept in `.tramalia/evidence/`. Details in the [ecosystem page](docs/ecosistema.md).

## 🗺️ Status & roadmap

**Implemented in preview (v0.6)** — verified with 34 pytest tests:
`init`, `doctor`, `detect`, **`close`** (gates → evidence + `metadata.json` → handoff, with an honest `status`), **`log`**, `evidence`, `handoff`, `gates`, `context`, `sync`, `skills`, `update`, `mcp`, `menu`. Template packaged in the wheel; bilingual docs site with diagrams; optional N2 memory via **Engram**; opt-in `--with-headroom`.

**Next / designed:** publish `tramalia-template` for `copier update`; interop with **Headroom** (compressed `review-summary.md` alongside the raw output, without replacing evidence); a `tramalia learn` command to import learnings into `docs/ai/06-intentos-fallidos.md`.

## 🤝 Contributing

Contributions are welcome. Open an issue to discuss large changes; for small ones, send a PR. Run the tests with `pip install -e ".[dev]" && pytest`.

## ❤ License & donations

License: **[Apache-2.0](LICENSE)** © 2026 Michael Jim Scott Bravo — permissive, with an explicit patent grant. All of Tramalia's dependencies are MIT, fully compatible.

**Donations:** accepted under any license, via GitHub Sponsors (configurable in [`.github/FUNDING.yml`](.github/FUNDING.yml)).

Full license analysis of the ecosystem — and why the copyleft of external tools doesn't affect Tramalia: [LICENSES.md](LICENSES.md).
