...
2. La escribe **cruda dentro del evidence pack** (`*-output.txt` + `gates-status.md`)
   y genera **`metadata.json`** (task, agente, reviewer, timestamps, exit codes, status).
3. Genera el **handoff**.
...
### `tramalia menu`
Menú interactivo (flechas ↑↓ con Questionary, o numerado en terminal básica).
Es el punto de entrada para no recordar cada comando.
...
- `docs/ai/` con las reglas curadas (incluye gates DB `03`, seguridad `04`, UX/UI `11`)
- `mise.toml` **a la medida del stack** (tools auto-update + tasks de gates)
- `.mcp.json` con Serena
- `.tramalia/config.json` y `.tramalia/skills.toml`
...
### `tramalia sync [--to copilot,cursor,cline]`
Fan-out de `AGENTS.md` a otros agentes con **rulesync**:
`rulesync convert --from agentsmd --to <targets> --features rules`. No incluye
Claude/Codex porque ya leen AGENTS.md nativamente. Genera, p. ej.,
...
**Implementado y verificado:** `doctor`, `detect`, `init`, `gates`, `context`,
`evidence`, `handoff`, `close`, `log`, `sync`, `skills`, `update`, `mcp`, `menu`.
25 tests con pytest. Wheel construible con la plantilla incluida. Memoria N2 opcional vía Engram.
