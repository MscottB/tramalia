...
**Implementado y verificado (v0.7):** `doctor`, `detect`, `init` (convención completa:
docs/ai 00–11, specs/, 13 skills), `gates`, `context`, `evidence`, `handoff`, `close`,
`log`, `sync`, `skills`, `update`, `mcp`, `ui` (dashboard TUI), `menu` (bucle + prompts).
47 tests con pytest. Wheel construible con la plantilla incluida. Interop: Engram (N2),
