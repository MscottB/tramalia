...
def test_mise_en_windows_winget_primero():
    opts = installer.options_for(_tool("mise"), os_name="windows")
...
def test_mise_en_linux_no_ejecuta_curl_pipe():
    opts = installer.options_for(_tool("mise"), os_name="linux")
    assert all(not o.auto for o in opts if "curl" in o.display)  # nunca automatizado


def test_uv_por_so():
    assert installer.options_for(_tool("uv"), os_name="windows")[0].method == "winget"
    assert installer.options_for(_tool("uv"), os_name="macos")[0].method == "brew"
...
def test_best_auto_respeta_disponibilidad(monkeypatch):
    ...
    assert installer.best_auto(_tool("repomix")) is None
