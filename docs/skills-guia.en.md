...
1. **See what's declared**: `tramalia skills list` — reads `.tramalia/skills.toml` (the manifest: *references*, not copies).
2. **Enable an external one**: open `.tramalia/skills.toml` and **uncomment** its `[[skill]]` block.
