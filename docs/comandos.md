...
`tramalia ui` abre un panel en la terminal (Textual, extra `pip install "tramalia-cli[tui]"`) con tres vistas: **Resumen** (doctor en vivo + gates aplicables), **Auditoría** (los cierres de `log`, navegables; Enter muestra el `metadata.json`) y **Cierre** (formulario tarea/agente/revisor + salida de gates). Solo lee e invoca el core — cero lógica nueva.
