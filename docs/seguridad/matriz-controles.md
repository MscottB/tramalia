...
|---|---|---|---|---|---|
| TRM-SEC-001 | Confinar toda ruta y rechazar traversal/symlinks fuera de raíz | [OWASP Top 10 2025](https://owasp.org/Top10/2025/0x00_2025-Introduction/); [ASVS 5.0.0](https://github.com/OWASP/ASVS/tree/v5.0.0) | cubierto_por_prueba | `uv run --no-sync pytest tests/unidad/test_seguridad_entradas.py tests/integracion/test_mcp_seguridad.py -q` | La cobertura se limita a fronteras locales de filesystem y lectura MCP. Reduce traversal, symlinks y reparse points, pero no elimina carreras TOCTOU frente a un proceso local concurrente con la misma autoridad. |
...
| TRM-SEC-005 | Validar habilidades antes de hacerlas visibles | [AST01/AST03/AST04/AST05/AST06/AST08](https://owasp.org/www-project-agentic-skills-top-10/) | cubierto_por_prueba | `uv run --no-sync pytest tests/integracion/test_habilidades_seguras.py tests/integracion/test_habilidades_git.py -q` | La cuarentena cubre estructura, modos Git, limites y publicacion transaccional; no certifica la semantica ni la conducta de una habilidad. Perfiles y permisos siguen en Plan 03c. |
| TRM-SEC-006 | Evitar colisiones, scope creep y sobreexposición MCP | [MCP02/MCP03/MCP07/MCP09/MCP10](https://owasp.org/www-project-mcp-top-10/) | cubierto_por_prueba | `uv run --no-sync pytest tests/integracion/test_mcp_seguridad.py tests/integracion/test_mcp_operaciones.py -q` | Cobertura limitada al proceso MCP local sobre stdio: hereda la autoridad de su usuario. No expone HTTP/SSE ni agrega autenticacion aparente; perfiles y gobernanza completa siguen en Plan 03c. |
