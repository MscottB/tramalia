...
- **Enter on an external one**: if absent, it installs it; if already installed, it explicitly updates that one skill (equivalent to `skills update <name>`).
- **`s` key** only rehydrates every declaration from its pinned SHA; it never moves Team locks.
...
There's also a **URL input**: paste any skill's Git URL and Enter adds it to the manifest; use Enter on it or `skills update <name>` to pin and materialize it.
...
CLI equivalents: `tramalia skills list` · `enable <name>` · `disable <name>` · `add <url>` · `sync [<name>]` (rehydrates) · `update [<name>]` (moves locks) · `outdated`.
